...
new Vue({
  ...
  computed: {
    filteredList() {
      return this.branches.filter(post => {
  ...
  mounted() {
    // console.log(this.profil.Token);
    // this.lengthTicket();
    this.startDate = moment().format('YYYYMMDD');
    // this.loadOS();
    // this.loadnumber();
    this.loadingOrg();
    // document.cookie = "token=asdsadas55; max-age=120;";
  ...
  methods: {
    ...
    openModal(info) {
      ...
      if(this.token == '') {
        ...
        this.branches = [];
        this.branches = this.info.filter(item => {
          if(item.ServiceName == info.ServiceName ) {
            return item
          }
        });
        
        this.eticketInfo.deviceType = 0;
        this.bookingInfo.serviseText = info.ServiceName;
        this.eticketInfo.serviseText = info.ServiceName;
      ...
      //   if(parseInt(this.info[index].GroupId) == id) {
      //     this.branches.push(this.info[index]);
      //   }
      // }
    ...
    loadingOrg() {
      ...
      // console.log(id);
      axios.get(`${this.api}/org?Id=${id}`)
        .then((responce) => {
          // console.log(responce.data);
          // console.log(responce.data[0]);
          this.loadingService(responce.data.ApiLink, responce.data.Id);
          this.apiLink = responce.data.ApiLink;
        })
    ...
    loadingService(api, id) {
      this.info = [];
      const local = localStorage.local;
      // axios.get(`${this.api}/${local}SearchOrg?OrgId=${id}`)
      axios.get(`${this.api}/${local}_search?OrgId=${id}`)
        .then((responce) => {
          const services = responce.data;
          for(let i=0; i < services.length; i++) {
            // let geo = Math.sqrt(Math.pow((parseFloat(this.services[i].Latitude) - this.location.Latitude), 2) + Math.pow((parseFloat(this.services[i].Longitude) - this.location.Longitude), 2)) * 100;
            const mas = {
              "OrgId": services[i].OrgId,
              "NameOrg": services[i].OrgName,
              "BranchId": services[i].BranchId,
              "BranchName": services[i].BranchName,
              "BranchAddress": services[i].BranchAddress,
              "BranchCoordinates": {
    ...
    openTwoTab(item) {
      ...
      this.BranchAddress = item.BranchAddress;

      this.OrgId = item.OrgId;
      this.OrgName = item.NameOrg;
      if(this.pickParams == 1) {
        this.loading = true;
        this.bookingModal = true;
        this.bookingInfo.serviseText = item.ServiceName;
      } else if(this.pickParams == 2){
        this.eticketModal = true;
        this.eticketInfo.serviseText = item.ServiceName;
      }
      const local = localStorage.getItem('local');
      axios.get(`${this.api}/service?OrgId=${item.OrgId}&BranchId=${item.BranchId}&Language=${local}`)
        .then((responce) => {
    ...
    loadDate() {
      this.loading = true;
      axios
        .get(`${this.apiLink}/ListFreeDaysMonth?branchId=${this.BranchId}&serviceId=${this.groupId}&startDate=${this.startDate}&languageShortId=${localStorage.local}`)
        .then((response) => {
    ...
    loadTimes() {
      ...
      this.timeAll = []
      axios
        .get(`${this.apiLink}/ListFreeTimes?branchId=${this.BranchId}&serviceId=${this.groupId}&startDate=${moment(this.date).format("YYYYMMDD")}&languageShortId=${localStorage.local}`)
        .then((response) => {
  ...
  watch: {
    ...
    date() {
      ...
      this.bookingInfo.date.month = moment(this.date).format("MM");
