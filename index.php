...
<head>
  ...
  <meta name="description" content="Данный сайт помогает вам найти услугу для онлайн регистрации на платформу">
  <meta name="keywords" content="Elektron-navbat, Elektron navbat, Поиск услуг" />
  <meta name="robots" content="All" />
  <link rel="icon" href="/img/favicon.ico">
  <title>Elektron-navbat | Портал электронной очереди</title>
...
<noscript><div><img src="https://mc.yandex.ru/watch/95583834" style="position:absolute; left:-9999px;" alt="" /></div></noscript>
<!-- /Yandex.Metrika counter -->
</head>
  ...
  <div id="home" class="wrapper">
    ...
    <div class="main">
      ...
      <section class="search">
        <div class="container">
          <h1 class="title"><?php echo $langArray['logoText'] ?></h1>
          <h2 class="subtitle"><?php echo $langArray['searchText'] ?></h2>
          <div class="search-input">
            <i aria-hidden="true" class="fa fa-search"></i>
            <input v-model="search" type="text" placeholder="<?php echo $langArray['search'] ?>">
          </div>
          <div v-if="search === ''" class="row">
          <?php foreach ($array as $item) : ?>
            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 col-12 mt">
              <a href="<?php echo ($lang == 'ru') ? '/org?id=' . $item['Id'] : '/' . $lang . '/org?id=' . $item['Id']?>" class="box">
                <!-- <div class="box__link"><?php echo $item['name'] ?></div> -->
          ...
          <div v-else class="lists">
            <ul class="row">
              <div v-for="item in filteredList" :key="item.index" class="col-xl-4 col-lg-6 col-md-6 col-sm-12 lists__item">
                <a @click.prevent="openModal(item)" class="lists__link" :style="{color: item.Color, background: item.Background}">{{ item.ServiceName }}
                  <span>{{ item.OrgName }}</span>
                  <span>{{ item.BranchName }}</span>
                  <span>{{ item.BranchAddress}}</span>
                  <span>{{ item.Geo }} км</span>
                </a>
              </div>
            </ul>
          </div>
            ...
            <div class="modal-info">
              <transition name="fade">
                <div v-show="pickModal" class="modal-info__pick">
                  <a @click.prevent="openOneTab(1)" href="" class="modal-info__link" :style="{color: Color, background: Background}"><?php echo $langArray['homeBtnOne'] ?></a>
                  <a @click.prevent="openOneTab(2)" href="" class="modal-info__link" :style="{color: Color, background: Background}"><?php echo $langArray['homeBtnTwo'] ?></a>
                </div>
              </transition>        
                          ...
                          <transition name="fade">
                            <div v-show="isBookingServiceActive" id="booking-form-service-list" class="form__service-list service-list">
                              ...
                              <ul id="booking-service-list" class="service-list__body">
                                <li v-for="item in service" :key="item.id" @click="liServiceActiveLink(item.GroupId)" class="service-list__item" :class="{ serviceListActive: serviceId == item.ServiceId }">
      ...
      <div v-if="call" class="call">
        <h1 class="call__title">Вас вызывают</h1>
        <div class="call__item">
          <p class="call__subtitle">Окно <span>5</span></p>
          <p class="call__subtitle">Номер <span>5</span></p>
        </div>
      </div>
    </div>
